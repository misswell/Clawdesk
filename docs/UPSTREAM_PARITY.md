...
| Session HUD | `SessionHUDWindowController` + `SessionHUDView` | Partial: click the pet to reveal a low-overhead native HUD near it, show up to three live sessions plus an overflow row, and click a row to focus its terminal; upstream pinning, hot-zone grace, and context-usage chips remain pending |
